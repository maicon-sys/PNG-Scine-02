...
const App: React.FC = () => {
  ...
  const handleRunDiagnosis = async () => {
    if (!activeProject || isDiagnosing) return;

    // FIX: A verificação de "matriz pronta" foi removida daqui.
    // O botão de Diagnóstico é a FERRAMENTA para CONSTRUIR a matriz,
    // então ele não pode ser bloqueado pela ausência dela.
    // A verificação permanece para as outras funções de IA (Gerar, Refinar).

    setIsDiagnosing(true);
    setDiagnosisLogs(["Iniciando análise de contexto para construir a Matriz Estratégica..."]);
    setDiagnosisStep(0);
